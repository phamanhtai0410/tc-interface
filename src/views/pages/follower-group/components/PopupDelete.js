...
const PopupDelete = ({isOpenModal, setIsOpenModal, groupName,fetchFollowerData}) => {
    const dispatch = useDispatch()
    const handleDelete = async()=>{
        const response = await dispatch(removeFollower(groupName.id))
  ...
  return (
    <Modal ariaHideApp={false} isOpen={isOpenModal} style={customStyles} className="text-black border-none overflow-y-auto fixed rounded-2xl p-[24px] w-[400px] max-w-[400px] bg-[#fff] top-[15%] left-1/2 -translate-x-1/2 z-50">
        <div>
            ...
            <p className='text-center text-[1rem] mb-8'>Are you sure you want to remove <span className='font-bold'>{groupName.name}</span> from Follower List?</p>
            <div className='flex justify-center gap-x-6'>
                <button onClick={handleDelete} className='px-4 py-2 bg-[#CF1322] text-white rounded font-semibold'>Yes, I'm sure</button>
